        ...
        keysToParse.forEach(key => {
            if (alohaConfiguration[key] && alohaConfiguration[key].filter) { // TODO: support object-based aloha configuration
                alohaConfiguration[key]
                    .filter(formattingId => registry.ckEditor.formattingAndStyling.has(formattingId))
                    .forEach(formattingId =>
                        allowedStylings.push(formattingId)
                    );
            }
        });
    }

    return allowedStylings;
};

export const calculateActiveFormattingRulesForNode = node => {
    const {properties} = node.nodeType;

    const activeFormattingRules = {};
    Object.keys(properties)
        .filter(inlineEditable(properties))
        .forEach(propertyName => {
            const property = properties[propertyName];
            activeFormattingRules[propertyName] = postProcessAlohaConfig($get('ui.aloha', property));
        });
 ...
 * {
 *    property1: [... list of registry.ckEditor.formattingAndStyling entries which should be enabled for the property ...]
 * }
 */
const currentlyEditedPropertyName = $get('ui.contentCanvas.currentlyEditedPropertyName');
const activeFormattingRules = createSelector(
    [
